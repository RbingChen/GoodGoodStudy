...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        if (np.random.random()<eps) | (self.t < self.learning_starts) :
        ...
        if perform_random_action:
            ...
            action = self.anv.action_space.sample()
        else:
            # HINT: Your actor will take in multiple previous observations ("frames") in order
                # to deal with the partial observability of the environment. Get the most recent 
                # `frame_history_len` observations using functionality from the replay buffer,
                # and then use those observations as input to your actor. 
            action = self.actor.get_actions(self.replay_buffer.encode_recent_observation())
        ...
        #TODO
        self.replay_buffer.store_effect(self.replay_buffer_idx,obs,reward,done)
        self.last_obs = obs
