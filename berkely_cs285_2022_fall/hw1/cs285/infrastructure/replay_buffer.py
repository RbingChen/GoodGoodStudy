...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        size_of_buffer = self.obs.shape[0]
        sample_indices = np.random.permutation(np.arrange(size_of_buffer))[-batch_size:]
        obs = self.obs[sample_indices]
        acs = self.acs[sample_indices]
        rews = self.rews[sample_indices]
        next_obs = self.next_obs[sample_indices]
